...
const lineMetricSpecs: MetricSpec[] = [
  { key: "cpu_mcores", title: "CPU Usage", subtitle: "", tone: "blue" },
  { key: "memory_bytes", title: "Memory Usage", subtitle: "", tone: "emerald" },
  { key: "network_rx_bytes", title: "Receive Throughput", subtitle: "", tone: "amber" },
  { key: "network_tx_bytes", title: "Send Throughput", subtitle: "", tone: "red" },
];

const breakdownMetricSpecs: MetricSpec[] = [
  { key: "request_cpu_cores", title: "CPU Requested", subtitle: "", tone: "blue" },
  { key: "limit_cpu_cores", title: "CPU Limit", subtitle: "", tone: "amber" },
  { key: "request_memory_bytes", title: "Memory Requested", subtitle: "", tone: "emerald" },
  { key: "limit_memory_bytes", title: "Memory Limit", subtitle: "", tone: "red" },
];
    ...
    >
      {!primarySeries ? (
        ...
        <div className="space-y-4">
          <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_172px]">
            ...
            <div className="space-y-3">
              <StatTile label="Latest" value={formatMetricValue(latest, primarySeries.unit)} tone={spec.tone} detail="Current" />
              <StatTile label="Peak" value={formatMetricValue(max, primarySeries.unit)} tone="amber" detail="Window" />
...
function NamespaceTable({
  ...
  return (
    <Panel title="Pods by Namespace" subtitle="Current scope" right={right}>
      {!rows.length ? (
...
function WorkloadTable({
  ...
  return (
    <Panel title="Workload Health" subtitle="Controllers and restarts" right={right}>
      {!rows.length ? (
...
export function ClusterDashboardView({
  ...
  const topPodCpuTrend = metricsOverview?.top_pods_by_cpu.map((item) => item.value) || [];
    ...
    <div className="space-y-4 text-[var(--text)]">
        ...
        <div className="flex flex-col gap-5">
          <div className="flex flex-col gap-4 2xl:flex-row 2xl:items-start 2xl:justify-between">
            <div className="min-w-0">
              ...
              <h1 className="mt-2 text-[30px] font-semibold tracking-tight text-[var(--text)]">Kubernetes Overview</h1>
              <p className="mt-2 max-w-2xl text-sm text-[var(--text-muted)]">High-level cluster health, runtime usage, and current operational pressure.</p>
            </div>
            <div className="flex flex-wrap items-center gap-2 text-xs text-[var(--text-soft)]">
              ...
              {metricsOverview?.collected_at ? <span className="rounded-full bg-[var(--bg-subtle)] px-2.5 py-1">{shortTimestamp(metricsOverview.collected_at)}</span> : null}
            </div>
          </div>
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6">
            <label className="flex min-w-0 flex-col gap-2">
              <span className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[var(--text-soft)]">Range</span>
              <select className="input h-10 min-w-0 border-[var(--border-strong)] bg-[var(--bg)]/60 text-sm text-[var(--text)]" value={selectedWindow} onChange={(event) => setSelectedWindow(event.target.value)} aria-label="Select dashboard time range">
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4 2xl:grid-cols-7">
        ...
        <DashboardMetricCard
          ...
          label="Pods"
          value={`${podCount}`}
          helper={`${healthyPodCount} healthy`}
          accent="#9b6bff"
          trend={topPodCpuTrend.length > 1 ? topPodCpuTrend : undefined}
        />
        <DashboardMetricCard
        ...
        <DashboardMetricCard
          ...
          label="Incidents"
          value={`${openIncidents}`}
          helper={`${criticalIncidents} critical`}
          accent="#ff5f56"
        />
        <DashboardMetricCard
          ...
          helper={selectedNamespace === "All" ? "All" : selectedNamespace}
          accent="#59d0ff"
        />
        <DashboardMetricCard
          icon={iconFor("cpu")}
          ...
          helper={metricsOverview?.collected_at ? shortTimestamp(metricsOverview.collected_at) : undefined}
          accent="#ffb020"
          trend={topNodeTrend.length > 1 ? topNodeTrend : undefined}
        />
        <DashboardMetricCard
          ...
          {podCount === 0 ? (
            ...
            <div className="space-y-4">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <MiddleStat label="Pods" value={`${podCount}`} detail="Current" tone="emerald" />
                <MiddleStat label="Incidents" value={`${openIncidents}`} detail="Open" tone="amber" />
              </div>
            </div>
          )}
        </Panel>
      ...
      <section className="grid gap-4 2xl:grid-cols-4">
        ...
        <RollupList title="Nodes CPU Usage" subtitle="Top nodes" items={metricsOverview?.top_nodes_by_cpu || []} />
        <RollupList title="Nodes Memory Usage" subtitle="Top nodes" items={metricsOverview?.top_nodes_by_memory || []} />
        <RollupList title="Top CPU Pods" subtitle="Current pods" items={metricsOverview?.top_pods_by_cpu || []} />
      </section>
      ...
      <section className="grid gap-4 2xl:grid-cols-4">
        ...
        >
          <div className="grid gap-3 sm:grid-cols-2">
            <StatTile label="CPU Requested" value={formatMetricValue(latestMetrics.request_cpu_cores?.total_value || 0, latestMetrics.request_cpu_cores?.unit)} tone="blue" detail="Requested" />
            <StatTile label="CPU Limit" value={formatMetricValue(latestMetrics.limit_cpu_cores?.total_value || 0, latestMetrics.limit_cpu_cores?.unit)} tone="amber" detail="Limit" />
            <StatTile label="Memory Requested" value={formatMetricValue(latestMetrics.request_memory_bytes?.total_value || 0, latestMetrics.request_memory_bytes?.unit)} tone="emerald" detail="Requested" />
            <StatTile label="Memory Limit" value={formatMetricValue(latestMetrics.limit_memory_bytes?.total_value || 0, latestMetrics.limit_memory_bytes?.unit)} tone="red" detail="Limit" />
          </div>
        </Panel>
        <Panel
          title="Recent Incidents"
          subtitle="Latest activity"
          right={<Link href={`/dashboard/clusters/${clusterId}/incidents`} className="text-xs font-medium text-[var(--primary)] hover:underline">View all</Link>}
        >
          {!recentIncidents.length ? (
            ...
            <div className="space-y-2.5">
              {recentIncidents.map((incident) => {
                ...
                return (
                  <Link key={incident.id} href={target} className="block rounded-xl border border-[var(--border)] bg-[var(--bg)]/55 px-3 py-3 transition hover:border-[var(--border-strong)] hover:bg-[var(--bg-subtle)]/60">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="shrink-0 text-right">
                        <p className="text-[11px] text-[var(--text-soft)]">{formatRelativeTime(incident.last_seen_at)}</p>
                      </div>
                    </div>
                  </Link>
                );
      ...
      <section className="grid gap-4 2xl:grid-cols-[1.05fr_0.95fr]">
        ...
        <Panel
          title="AI Insights"
          subtitle="Incident-backed summaries"
          right={<Link href={`/dashboard/clusters/${clusterId}/ai`} className="text-xs font-medium text-[var(--primary)] hover:underline">Open AI</Link>}
        >
              ...
              {aiInsightItems.map((incident) => (
                <div key={incident.id} className="rounded-xl border border-[var(--border)] bg-[var(--bg)]/55 px-4 py-3">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <p className="shrink-0 text-[11px] text-[var(--text-soft)]">{formatRelativeTime(incident.last_seen_at)}</p>
                  </div>
                </div>
              ))}
            </div>
